 ...
 **/
 $(document).ready(function(){
  ...
  $(".add_device").click(function(){
    $('#device').append("<div class='form-group'>"+dhtml+"</div><p><a href='#' class='btn btn-danger btn-xs delete_device'>Delete</a></p>");
    update_sidebar();
    if($().chosen) {
      $("select").chosen({disable_search_threshold: 5});
    }
  ...
  $('select#theme_files').chosen().change(function(e,v){
      window.location=appurl+"/themes/editor?file="+$("select#theme_files").val();
  });
  /**
   * Update Themes
   **/
 ...
 //Back to top
  ...
  $("input#name").blur(function() {
	  if ($("input#slug").val() == '') {
		  $("input#slug").val( $("input#name").val().toSlug() );
	  }
  });

  $("input#title").blur(function() {
	  if ($("input#slug").val() == '') {
		  $("input#slug").val( $("input#title").val().toSlug() );
	  }
  });
...
window.paymentProcess = function(v){
  $(".toggles").hide();
  $("#"+v+"holder").slideDown();
}
