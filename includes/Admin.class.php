...
class Admin{
  ...
  protected function blog(){
    ...
    private function blog_add(){
      ...
      // Add CDN Editor
      Main::cdn("ckeditor","",1);
      Main::admin_add("<script>CKEDITOR.plugins.addExternal('image2', '/admin/static/ckeditor/plugins/image2/plugin.js');CKEDITOR.plugins.addExternal('uploadimage', '/admin/static/ckeditor/plugins/uploadimage/plugin.js');CKEDITOR.plugins.addExternal('uploadwidget', '/admin/static/ckeditor/plugins/uploadwidget/plugin.js');CKEDITOR.plugins.addExternal('widget', '/admin/static/ckeditor/plugins/widget/plugin.js');CKEDITOR.plugins.addExternal('filetools', '/admin/static/ckeditor/plugins/filetools/plugin.js');CKEDITOR.plugins.addExternal('notificationaggregator', '/admin/static/ckeditor/plugins/notificationaggregator/plugin.js');CKEDITOR.plugins.addExternal('lineutils', '/admin/static/ckeditor/plugins/lineutils/plugin.js');CKEDITOR.plugins.addExternal('widgetselection', '/admin/static/ckeditor/plugins/widgetselection/plugin.js');CKEDITOR.plugins.addExternal('notification', '/admin/static/ckeditor/plugins/notification/plugin.js');</script>","custom",1);
      Main::admin_add("<script>CKEDITOR.replace( 'editor', {height: 350, extraPlugins: 'image2,uploadimage'});</script>","custom",1);
      $header="Add a Post";
      $content="
  ...
  protected function pages(){
    ...
    $this->footer();
      ...
      private function pages_add(){
        ...
        // Add CDN Editor
        Main::cdn("ckeditor","",1);
        Main::admin_add("<script>CKEDITOR.replace( 'editor', {height: 350, extraPlugins:'easyimage'});</script>","custom",1);
  ...
  protected function languages_add(){
    ...
    if(isset($_POST["token"])){
      ...
      foreach ($_POST["text"] as $o => $t) {
        fwrite($handle, "\n\"".strip_tags(str_replace('"', "&quot;", $o),"<br><b><i><s><u><strong>")."\"".'=>'."\"".strip_tags(str_replace('"', "&quot;", $t),"<br><b><i><s><u><strong>")."\",");
      }
      if(isset($_POST["new"]["original"]) && !empty($_POST["new"]["original"])){
        fwrite($handle, "\n\"".strip_tags($_POST["new"]["original"],"<br><b><i><s><u><strong>")."\"".'=>'."\"".strip_tags(str_replace('"', "&quot;", $_POST["new"]["translated"]),"<br><b><i><s><u><strong>")."\",");
      }
      fwrite($handle, "); ?>");
      fclose($handle);
    ...
    if(!file_exists(ROOT."/includes/languages/lang_sample.php")){
      ...
      foreach ($lang as $original => $translation){
        $content.= "<div class='col-md-4'><div class='form-group'>
          ...
          <textarea name=\"text[".htmlentities($original)."]\" class='form-control' style='min-height:60px;'>".htmlentities($translation)."</textarea>
        </div></div>";
        $i++;
  ...
  protected function languages_edit(){
    ...
    if(isset($_POST["token"])){
      ...
      if(isset($_POST["new"]["original"]) && !empty($_POST["new"]["original"])){
        fwrite($handle, "\n\"".strip_tags($_POST["new"]["original"],"<br><b><i><s><u><strong>")."\"".'=>'."\"".strip_tags($_POST["new"]["translated"],"<br><b><i><s><u><strong>")."\",");
      }
      fwrite($handle, "); ?>");
      fclose($handle);
